/**
 * tailwindColorMatch.js
 * Finds the nearest Tailwind CSS color name + shade for a given hex.
 */
...
export function findNearestTailwindColor(hex) {
  ...
  for (const [name, shades] of Object.entries(TW_COLORS)) {
    for (let i = 0; i < shades.length; i++) {
      const dist = colorDistance(hex, shades[i]);
      if (dist < bestDist) {
        bestDist = dist;
        bestName = name;
        bestShade = TW_SHADES[i];
      }
    }
  }

  return { name: bestName, shade: bestShade, class: `${bestName}-${bestShade}` };
}

export function formatColor(hex, format, rgb, hsl) {
  switch (format) {
    ...
    case "CSS Var": return `var(--color-primary)`; // generic example
    case "Tailwind": {
      const tw = findNearestTailwindColor(hex);
      return `${tw.class}`;
    }
